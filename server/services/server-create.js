...
import { getConfig, getClient, getKernelEnvironment } from "./sync.js";

const clientCache = new Map();

/**
 * Создать сервер Pterodactyl используя данные из БД
 */
export async function createPteroServer({
  ...
  // 2. Получаем переменные окружения из БД
  let finalEnvironment = {};

  if (kernelId) {
    finalEnvironment = await getKernelEnvironment(kernelId);
  }
