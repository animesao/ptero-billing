  ...
  try {
    ...
    if (sanitizedUsername.length < 2) {
      return res.status(400).json({
        error: 'Имя пользователя должно содержать минимум 2 допустимых символа (буквы, цифры, тире, подчёркивания, точки)',
    ...
    const existingUsername = await db.select().from(users).where(eq(users.username, sanitizedUsername)).limit(1);
    ...
    try {
      ...
      console.log('Pterodactyl not configured or connection failed, skipping Ptero user creation');
    }
    ...
    if (pteroConfigured) {
      try {
        ...
        const isDuplicateEmail = pteroErrors.some(e =>
          e.detail?.toLowerCase().includes('email has already been taken') ||
          e.detail?.toLowerCase().includes('email уже')
        );
        const isDuplicateUsername = pteroErrors.some(e =>
          e.detail?.toLowerCase().includes('username has already been taken') ||
          e.detail?.toLowerCase().includes('username уже')
        );
        const isInvalidUsername = pteroErrors.some(e =>
          e.detail?.toLowerCase().includes('username must start') ||
          e.detail?.toLowerCase().includes('username') && e.detail?.toLowerCase().includes('contain only')
        );

        if (isDuplicateEmail) {
          return res.status(400).json({ error: 'Email уже зарегистрирован в Pterodactyl' });
        }
        if (isDuplicateUsername) {
          return res.status(400).json({ error: 'Имя пользователя уже занято в Pterodactyl' });
        }
        if (isInvalidUsername) {
          return res.status(400).json({
            error: 'Имя пользователя содержит недопустимые символы. Разрешены только буквы, цифры, тире, подчёркивания и точки.',
            suggestion: sanitizedUsername + '_user'
          });
        }

        // Логируем другие ошибки но не прерываем регистрацию
        console.error('Pterodactyl registration error:', pteroData || pteroError.message);
      }
    }
