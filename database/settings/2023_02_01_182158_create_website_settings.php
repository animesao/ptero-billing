...
class CreateWebsiteSettings extends LegacySettingsMigration
{
    public function up(): void
    {
        ...
        $this->migrator->add(
            "website.motd_enabled",
            $table_exists
                ? $this->getOldValue("SETTINGS::SYSTEM:MOTD_ENABLED", true)
                : true,
        );
        $this->migrator->add(
            "website.motd_message",
            $table_exists
                ? $this->getOldValue("SETTINGS::SYSTEM:MOTD_MESSAGE")
                : "<h1 style='text-align: center;'><img style='display: block; margin-left: auto; margin-right: auto;' src='/images/ptero-billing-logo.png' alt='' width='200' height='200'><span style='font-size: 36pt;'>Ptero-Billing</span></h1>
 ...
 <p><span style='font-size: 10pt;'>(you can change this message in the <a href='admin/settings#system'>Settings</a> )</span></p>",
        ...
        $this->migrator->add(
            "website.show_imprint",
            $table_exists
                ? $this->getOldValue("SETTINGS::SYSTEM:SHOW_IMPRINT", false)
                : false,
        );
        $this->migrator->add(
            "website.show_privacy",
            $table_exists
                ? $this->getOldValue("SETTINGS::SYSTEM:SHOW_PRIVACY", false)
                : false,
        );
        $this->migrator->add(
            "website.show_tos",
            $table_exists
                ? $this->getOldValue("SETTINGS::SYSTEM:SHOW_TOS", false)
                : false,
        );
        $this->migrator->add(
            "website.useful_links_enabled",
            $table_exists
                ? $this->getOldValue(
                    "SETTINGS::SYSTEM:USEFULLINKS_ENABLED",
                    true,
                )
                : true,
        );
        $this->migrator->add(
            "website.seo_title",
            $table_exists
                ? $this->getOldValue("SETTINGS::SYSTEM:SEO_TITLE")
                : "Ptero-Billing",
        );
        $this->migrator->add(
            "website.seo_description",
            $table_exists
                ? $this->getOldValue("SETTINGS::SYSTEM:SEO_DESCRIPTION")
                : "Billing software for Pterodactyl Panel.",
        );
    ...
    public function down(): void
    {
        DB::table("settings_old")->insert([
            ...
            [
                ...
                "type" => "text",
                "description" =>
                    "The message that will be displayed in the MOTD.",
            ],
            [
